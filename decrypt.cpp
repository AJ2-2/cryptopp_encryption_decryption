...
{
    string mode="2",fileName=argv[1],sKey="Password_key1234";
    ...
    if(mode=="1")
    {
      ...
      ofstream oFile(fileName+".aes.encrypt", ios::binary|ios::out);
      size_t len = ciphertext.size();
      oFile.write(ciphertext.c_str(), len);
      oFile.close();
      cout<<"AES encypted file: "<<fileName<<" -> "<<fileName<<".aes.encrypt\n";
    }
    else if(mode=="2")
    {      
